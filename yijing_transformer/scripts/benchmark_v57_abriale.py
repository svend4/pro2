#!/usr/bin/env python3
"""
v57 Benchmark: Абриале — событийно-управляемые изотропные N-местные связи.

По мотивам А.И. Пацкина (РосНИИ ИИ, Диалог 2003):
...
Fair-parametric сравнение при одинаковом бюджете параметров:
  ...
  2. yijing_binary   — бинарная квантизация {-1,+1}⁶
  3. abriale_binary  — + Абриале (бинарные связи, 64 правила)
  4. abriale_ternary — + Абриале (тернарные связи, 64 правила)
  5. full_v57        — Абриале + ConvergenceBridge + MatrixGrammar

Данные: WikiText-2 (byte-level) или synthetic fallback.
Метрики: val perplexity, train loss, параметры, время,
         а также: симметрия attention, commit rate, энтропия хитов.
"""
...
CONFIGS = {
    ...
    'abriale_binary': dict(
        ...
        abriale_n_rules=64,
        abriale_n_hits=4,
        abriale_n_alternatives=2,
        abriale_n_event_types=8,
    ),
    'abriale_ternary': dict(
        hex_strength=0.05,
        quantizer_type='factored6',
        use_abriale=True,
        abriale_arity=3,          # тернарные N-местные связи
        abriale_n_rules=64,
        abriale_n_hits=4,
        abriale_n_alternatives=2,
        abriale_n_event_types=8,
...
def train_and_eval(cfg, name, train_data, val_data,
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % eval_every == 0 or step == 1:
            ...
            # Abriale diagnostics
            extra = ""
            if getattr(cfg, 'use_abriale', False) and hasattr(model, 'abriale'):
                extra += f" scale={model.abriale.scale.item():.4f}"
                extra += f" T={model.abriale.rule_bank.temperature.item():.3f}"
    ...
    extra_info = {}
    if getattr(cfg, 'use_abriale', False) and hasattr(model, 'abriale'):
        extra_info['abriale_scale'] = model.abriale.scale.item()
        extra_info['abriale_temperature'] = model.abriale.rule_bank.temperature.item()
        # Measure symmetry and commit rate on a batch
        model.eval()
        with torch.no_grad():
            x, _ = get_batch(train_data, cfg.block_size, batch_size, device)
            emb = model.tok_emb(x)
            if model.pos_emb is not None:
                emb = emb + model.pos_emb[:, :cfg.block_size, :]
            _, info = model.abriale(emb)
            extra_info['attn_symmetry'] = info['attn_symmetry']
            extra_info['commit_rate'] = info['commit_rate']
            extra_info['hit_entropy'] = info['hit_entropy']
            extra_info['event_type_entropy'] = info['event_type_entropy']
...
def main():
    ...
    print("=" * 70)
    print("v57 Benchmark: Абриале — событийно-управляемые изотропные связи")
    print("  (А.И. Пацкин, РосНИИ ИИ, Диалог 2003)")
    print("=" * 70)
    ...
    configs_to_run = ['vanilla', 'yijing_binary', 'abriale_binary',
                      'abriale_ternary', 'full_v57']
    ...
    # Summary table
    print("\n" + "=" * 90)
    print("SUMMARY: v57 Abriale Benchmark")
    print("=" * 90)
    print(f"{'Config':<20} {'Params':>10} {'Best PPL':>10} {'Final PPL':>10} {'Time':>8} {'Δ PPL':>8} {'Symmetry':>10}")
    print("-" * 90)
    ...
    for r in results:
        ...
        sym = f"{r.get('attn_symmetry', '-'):.4f}" if isinstance(r.get('attn_symmetry'), float) else "-"
        print(f"{r['name']:<20} {r['params']:>10,} {r['best_ppl']:>10.1f} {r['final_ppl']:>10.1f} {r['elapsed']:>7.1f}s {sign}{delta:>7.1f} {sym:>10}")

    # Abriale-specific summary
    abriale_results = [r for r in results if 'abriale_scale' in r]
    if abriale_results:
        print(f"\n{'='*70}")
        print("ABRIALE DIAGNOSTICS")
        print(f"{'='*70}")
        print(f"{'Config':<20} {'Scale':>8} {'CommitR':>10} {'HitEntr':>10} {'EvtEntr':>10}")
        print(f"{'-'*70}")
        for r in abriale_results:
            print(f"{r['name']:<20} {r.get('abriale_scale', 0):.4f} "
                  f"{r.get('commit_rate', 0):>10.4f} "
                  f"{r.get('hit_entropy', 0):>10.4f} "
                  f"{r.get('event_type_entropy', 0):>10.4f}")

    # Save results
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            '..', 'benchmark_v57_results.json')
