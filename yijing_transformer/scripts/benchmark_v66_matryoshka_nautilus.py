#!/usr/bin/env python3
"""
v66 MatryoshkaNautilus Benchmark: spacetime encoding between Nautilus chambers.

Ключевая идея: MatryoshkaQuantizer Level 2 (пространство×время) получает
реальный временной сигнал — разницу между состоянием до и после каждой
камеры Наутилуса. В v65 квантизатор был изолирован (x_ref ≡ x → gate→0.5).
Теперь x_ref ≠ x, и модель может выучить, что изменила каждая камера.

Configs:
  1. vanilla             — чистый трансформер (baseline)
  2. nautilus_only       — NautilusHierarchy без matryoshka (v63 baseline)
  3. matryoshka_naut_seq — MatryoshkaNautilus sequential (все 7 камер)
  4. matryoshka_naut_2ch — MatryoshkaNautilus, 2 камеры (heisenberg + flower_gat)
  5. matryoshka_naut_par — MatryoshkaNautilus parallel (все 7 камер)

Гипотезы:
  H1: matryoshka_naut_seq < nautilus_only (spacetime кодирование добавляет информацию)
  H2: matryoshka_naut_2ch < vanilla (даже 2 камеры с matryoshka лучше baseline)
  H3: matryoshka_naut_seq < matryoshka_naut_par (sequential даёт каскад x_ref)

Usage:
  python benchmark_v66_matryoshka_nautilus.py --group all
  python benchmark_v66_matryoshka_nautilus.py --run matryoshka_naut_seq
  python benchmark_v66_matryoshka_nautilus.py --summary
"""
...
RESULTS_PATH = os.path.join(
    ...
    'benchmark_v66_matryoshka_nautilus_results.json'
)
...
class NautilusLM(nn.Module):
    """Language model with optional Nautilus/MatryoshkaNautilus enrichment.

    Architecture:
        Embedding → [TransformerBlock × n_layers/2]
                  → Enrichment (NautilusHierarchy or MatryoshkaNautilus)
    ...
    def __init__(self, vocab_size, d_model=128, n_layers=4, n_heads=4,
        ...
        # Transformer layers split in two
        n_first = n_layers // 2
...
CONFIGS = {
    ...
    'matryoshka_naut_seq': dict(
        enrichment='matryoshka_nautilus',
        enrichment_kwargs=dict(mode='sequential', warmup_steps=200),
    ),
    'matryoshka_naut_2ch': dict(
        enrichment='matryoshka_nautilus',
        enrichment_kwargs=dict(
            mode='sequential', warmup_steps=200,
            enabled_chambers=['heisenberg', 'flower_gat'],
        ),
    ),
    'matryoshka_naut_par': dict(
        enrichment='matryoshka_nautilus',
        enrichment_kwargs=dict(mode='parallel', warmup_steps=200),
    ),
}

CONFIG_ORDER = ['vanilla', 'nautilus_only', 'matryoshka_naut_seq',
                'matryoshka_naut_2ch', 'matryoshka_naut_par']
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % eval_every == 0 or step == 1:
            ...
            if stats:
                ...
                if rg is not None:
                    extra += f" rg={rg:.3f}"
                # Show matryoshka gates if present
                m_gates = [v for k, v in stats.items() if k.startswith('matryoshka/') and k.endswith('/gate')]
                if m_gates:
                    avg_mg = sum(m_gates) / len(m_gates)
                    extra += f" m_gate_avg={avg_mg:.3f}"
...
def print_summary():
    ...
    print("\n" + "=" * 95)
    print("  v66 MATRYOSHKA-NAUTILUS BENCHMARK: Spacetime Encoding Between Chambers")
    print("=" * 95)
    ...
    mn_seq = results.get('matryoshka_naut_seq', {}).get('best_ppl')
    mn_2ch = results.get('matryoshka_naut_2ch', {}).get('best_ppl')
    mn_par = results.get('matryoshka_naut_par', {}).get('best_ppl')

    if mn_seq is not None and naut is not None:
        print(f"  H1 (matryoshka_seq < nautilus_only):   {'CONFIRMED' if mn_seq < naut else 'REJECTED'}"
              f"  ({mn_seq:.2f} vs {naut:.2f})")
    if mn_2ch is not None and vanilla_ppl is not None:
        print(f"  H2 (matryoshka_2ch < vanilla):         {'CONFIRMED' if mn_2ch < vanilla_ppl else 'REJECTED'}"
              f"  ({mn_2ch:.2f} vs {vanilla_ppl:.2f})")
    if mn_seq is not None and mn_par is not None:
        print(f"  H3 (sequential < parallel):            {'CONFIRMED' if mn_seq < mn_par else 'REJECTED'}"
              f"  ({mn_seq:.2f} vs {mn_par:.2f})")
...
def main():
    parser = argparse.ArgumentParser(description='v66 MatryoshkaNautilus Benchmark')
    ...
    if args.run:
        ...
        if args.group == 'baselines':
            ...
            names = ['matryoshka_naut_seq', 'matryoshka_naut_2ch', 'matryoshka_naut_par']
        else:
            names = CONFIG_ORDER
        for name in names:
