...
ОГРАНИЧЕНИЕ: 76 символов SOLAN не вмещаются в 64 вершины Q6.
Буквы имеют приоритет при декодировании (decode) — цифры и
спецсимволы, попавшие на тот же код, будут декодированы как буква.
Это означает encode→decode НЕ является round-trip для цифр и
...
class GlyphTokenizer:
    ...
    def __init__(self):
        ...
        # Обратный маппинг: приоритет lowercase > uppercase > digits > special
        # При дублировании кодов, буквы побеждают (чаще встречаются в тексте)
        priority_order = (
            list(_LOWER.items()) +     # lowercase first
            list(_UPPER.items()) +     # then uppercase
            list(_SPECIAL.items()) +   # then special
            list(_DIGITS.items())      # digits last
        )
        for ch, bits in reversed(priority_order):
    ...
    def encode(self, text: str) -> Tensor:
        ...
        for ch in text:
            ...
            if bits is None:
                # Fallback: ord(ch) для детерминированного маппинга
                code = ord(ch) % 64
                bits = tuple((code >> (5 - b)) & 1 for b in range(6))
    ...
    def decode(self, vertices: Tensor) -> str:
        ...
        chars = []
        for v in vertices:
            # Квантизация: threshold at 0 → {0, 1} bits
            bits = tuple(int(x) for x in (v > 0).long().tolist())
