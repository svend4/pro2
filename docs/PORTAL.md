...
Большинство Portal-функций работают **без клонов** (автоматический fallback).
Для полного функционала клонируйте смежные репозитории рядом с `pro2`:

```bash
cd ..
git clone https://github.com/svend4/meta.git    # MetaAdapter, meta_q6, meta_bridge
git clone https://github.com/svend4/info1.git   # Info1Adapter
# data2, data7 — опциональные (ЕТД Крюкова, энциклопедии)
```

**Fallback-поведение при отсутствии клона:**

| Компонент | Без клона | С клоном |
|-----------|-----------|---------|
| `MetaAdapter` | возвращает пустой список, `is_available()=False` | CA-правила → 64 гексаграммы |
| `meta_q6` | работает полностью (встроен в pro2) | — |
| `CorpusLoader` | загружает только локальные данные pro2 | + данные всех клонов |
| `federated_round.py` | пропускает недоступные репо | синхронизирует веса |
| `Info1Adapter` | `is_available()=False`, пропускается | α-уровни Markdown |

> Все адаптеры реализуют `is_available() → bool` — проверяйте перед вызовом `fetch()`.
